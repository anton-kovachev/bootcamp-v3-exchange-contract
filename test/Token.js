...
describe("Token", () => {
  ...
  describe("Sending Tokens", () => {
    it("Transfer token balances", async () => {
      const { token, deployer, receiver } = await loadFixture(
        deployTokenFixture
      );
    ...
    it("reject insufficient balances", async () => {
      ...
      await expect(
        token.connect(deployer).transfer(receiver.address, INVALID_AMOUNT)
    ...
    it("reject invalid recipient", async () => {
      ...
      await expect(
        token.connect(deployer).transfer(INVALID_ADDRESS, tokens("100"))
      ).to.be.revertedWith("Token: recipient is address 0.");
    });
  });
  ...
  describe("Approving Tokens", () => {
    describe("Success", () => {
      it("allocates an allowance for delegated token spending", async () => {
        ...
        const { token, deployer, exchange } = await loadFixture(
          deployTokenFixture
        );
  ...
  describe("Delegated Token Transfers", () => {
    ...
    describe("Success", () => {
      ...
      it("resets the allowance", async () => {
        const { token, exchange, deployer } = await loadFixture(
          transferFromTokenFixture
        );
      ...
      it("emits a transfer event", async () => {
        const { token, transaction, deployer, receiver } = await loadFixture(
          transferFromTokenFixture
        );
    ...
    describe("Failure", () => {
      it("reject insufficient funds", async () => {
        ...
        const { token, deployer, receiver, exchange } = await loadFixture(
          transferFromTokenFixture
        );
        ...
        const transaction = token
          .connect(exchange)
          .transferFrom(deployer.address, receiver.address, LARGE_AMOUNT);
      ...
      it("reject from address invalid", async () => {
        ...
        const transaction = token
          .connect(exchange)
          .transferFrom(INVALID_ADDRESS, receiver.address, AMOUNT);
        await expect(transaction).to.be.revertedWith(ERROR);
      });
      ...
      it("reject to address invalid", async () => {
        ...
        const transaction = token
          .connect(exchange)
          .transferFrom(deployer.address, INVALID_ADDRESS, AMOUNT);
        await expect(transaction).to.be.revertedWith(ERROR);
      });
    });
  });
