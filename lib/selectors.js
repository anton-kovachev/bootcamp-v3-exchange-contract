...
export const flashLoans = (state) => _.get(state, "exchange.flashLoans");
export const selectFlashLoans = createSelector(flashLoans, (flashLoans) =>
  flashLoans
    .map(({ token, amount, timestamp }) => ({
      token,
      amount,
      date: moment.unix(timestamp.toString()).format("D MMM YY hh:mm:ss"),
    }))
    .reverse()
);
