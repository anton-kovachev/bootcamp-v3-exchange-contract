...
export const selectTokenBalances = (state) => _.get(state, "tokens.balances", {});
...
export const selectAllOrders = (state) => _.get(state, "exchange.allOrders", []);
export const selectCancelledOrders = (state) => _.get(state, "exchange.cancelledOrders", []);
export const selectFilledOrders = (state) => _.get(state, "exchange.filledOrders", []);
...
export const selectOpenOrdersFromMarket = createSelector(selectAllOpenOrders, selectMarket, (orders, market) => {
    if (!market)
    ...
    return { orders, buyOrders: _.get(grouped_orders, "buy", []), sellOrders: _.get(grouped_orders, "sell", []) };
})

export const selectFilledOrdersFromMarket = createSelector(selectFilledOrders, selectMarket, (filledOrders, market) => {
...
export const selectAccountOpenOrders = createSelector(selectAccount, selectOpenOrdersFromMarket, (account, openOrders) => {
    const accountOrders = _.reject(openOrders.orders, (x) => (x.user != account));
    return accountOrders;
})

export const selectAccountFilledOrders = createSelector(selectAccount, selectFilledOrdersFromMarket, (account, filledOrders) => {
    const accountFilledOrders = _.reject(filledOrders, (x) => (x.user != account));
    return accountFilledOrders;
})

export const selectPriceData = createSelector(selectFilledOrdersFromMarket, (orders) => {
    const sortedOrders = orders.sort((a, b) => (a.timestamp - b.timestamp));
    const [secondLastOrder, lastOrder] = sortedOrders.slice(sortedOrders.length - 2);
    ...
    return { lastPrice, lastPriceChange: lastPrice >= secondLastPrice ? "+" : "-", series: [{ data: graphData }] };

})

const buildGraphData = (orders) => {
    orders = _.groupBy(orders, (x) => (moment.unix(x.timestamp).startOf("hour").format()));
