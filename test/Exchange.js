...
describe("Exchange", () => {
  describe("Deployment", () => {
    it("", async () => {
      const { exchange, accounts, feePercent } = await loadFixture(
        deployExchangeFixture
      );
  ...
  describe("Depositing Tokens", () => {
    describe("Success", () => {
      it("tracks the token deposit", async () => {
        ...
        expect(await token0.balanceOf(await exchange.getAddress())).to.equal(
          AMOUNT
        );
        expect(
          await exchange.totalBalanceOf(
        ...
        expect(await token1.balanceOf(await exchange.getAddress())).to.equal(
          AMOUNT
        );
        expect(
          await exchange.totalBalanceOf(
      ...
      it("emits a TokenDeposited event", async () => {
        ...
        await expect(transaction)
          .to.emit(exchange, "TokensDeposited")
          .withArgs(await token0.getAddress(), user1.address, AMOUNT, AMOUNT);
      });
    });
    ...
    describe("Failure", () => {
      it("fails when no tokens are approved", async () => {
        ...
        const transaction = exchange
          .connect(user1)
          .depositToken(await token0.getAddress(), AMOUNT);
  ...
  describe("Withdraw tokens", () => {
    describe("Success", () => {
      ...
      it("emits a token withdraw event", async () => {
        ...
        await expect(transaction)
          .to.emit(exchange, "TokensWithdraw")
          .withArgs(await token0.getAddress(), user1.address, AMOUNT, 0);
      });
    });
  ...
  describe("Cancel Order", () => {
    describe("Success", () => {
      ...
      it("active balance of", async () => {
        ...
        expect(
          await exchange.activeBalanceOf(
            await token0.getAddress(),
            user1.address
          )
        ).to.equal(0);
    ...
    describe("Success", () => {
      it("executes trades and charges fees", async () => {
        const {
          tokens: { token0, token1 },
          exchange,
          accounts: { user1, user2, feeAccount },
        } = await loadFixture(orderExchangeFixture);
        const ORDER_ID = 1;
        ...
        expect(await exchange.totalBalanceOf(user1.address, token0)).to.equal(
          REMAINING_AMOUNT
        );
        expect(await exchange.totalBalanceOf(user1.address, token1)).to.equal(
          GET_AMOUNT
        );
        expect(await exchange.activeBalanceOf(user1.address, token0)).to.equal(
          0
        );

        expect(await exchange.totalBalanceOf(feeAccount, token0)).to.equal(0);
        expect(await exchange.totalBalanceOf(feeAccount, token1)).to.equal(
          calculateFees(GET_AMOUNT, FEE_PERCENT)
        );

        expect(await exchange.totalBalanceOf(user2.address, token0)).to.equal(
          GIVE_AMOUNT
        );
        expect(await exchange.totalBalanceOf(user2.address, token1)).to.equal(
          REMAINING_AMOUNT - calculateFees(GET_AMOUNT, FEE_PERCENT)
        );
      });
