...
async function deployExchangeFixture() {
  ...
  return {
    ...
    exchange,
    accounts: { deployer, feeAccount, user1, user2 },
...
async function depositExchangeFixture() {
  ...
  await (
    await tokens.token0
      ...
      .approve(await exchange.getAddress(), AMOUNT)
  ).wait();
  const transaction = await exchange
    .connect(accounts.user1)
    .depositToken(await tokens.token0.getAddress(), AMOUNT);
  ...
  await (
    await exchange
      .connect(accounts.user2)
      .depositToken(await tokens.token1.getAddress(), AMOUNT)
  ).wait();
